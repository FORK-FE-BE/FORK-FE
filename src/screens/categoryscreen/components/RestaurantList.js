...
// 실제 파일을 아래 경로에 미리 넣어두세요.
// 예: src/assets/images/ham1.jpg, ham2.jpg, ham3.jpg, ham4.jpg, ham5.jpg, ham6.jpg ...
...
// 식당 id를 seed로 해서 4장 선택 (풀 개수가 4 미만이어도 순환)
function pickFourLocal(id) {
  const n = ALL_LOCAL_DUMMIES.length;
  if (n === 0) return []; // 파일이 아직 없을 때
  const start = (Number(id) * 4) % n;
  return [
    ALL_LOCAL_DUMMIES[start % n],
    ALL_LOCAL_DUMMIES[(start + 1) % n],
    ALL_LOCAL_DUMMIES[(start + 2) % n],
    ALL_LOCAL_DUMMIES[(start + 3) % n],
  ];
}

// (옵션) 원격 더미 (사용하지 않지만 필요 시 켜서 테스트 가능)
const getDummyRemotePhotos = (id) => [
  `https://picsum.photos/seed/rs-${id}-1/260/260`,
  `https://picsum.photos/seed/rs-${id}-2/260/260`,
  `https://picsum.photos/seed/rs-${id}-3/260/260`,
  `https://picsum.photos/seed/rs-${id}-4/260/260`,
];
// ======================================================================
...
const styles = StyleSheet.create({
  ...
  fullscreen: {
    flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#fff', width: '100%', height: '100%',
  },
  restaurantItem: { marginBottom: 17 },
  nameRatingRow: { paddingHorizontal: 24, flexDirection: 'row', marginTop: 10 },
  restaurantName: { fontSize: 16, fontFamily: 'Paperlogy-Medium' },
  restaurantRating: { textAlignVertical: 'center', marginLeft: 10, fontSize: 14, marginBottom: 10, fontFamily: 'Paperlogy-Medium' },
  star: { color: '#FFD900', fontSize: 16, fontFamily: 'Paperlogy-Medium' },
  restaurantReview: { marginLeft: 3, marginTop: 2, fontSize: 13, marginBottom: 10, fontFamily: 'Paperlogy-Medium', color: '#979797' },
  contentWrap: { flexDirection: 'row', marginBottom: 17 },
  arcontent: {
    backgroundColor: '#EDF4FF', borderRadius: 3, width: 47, height: 20, fontSize: 10, textAlign: 'center',
    color: '#006DF0', lineHeight: 20, marginLeft: 24, fontFamily: 'Paperlogy-SemiBold'
  },
  coupon: {
    backgroundColor: '#EDF4FF', borderRadius: 3, width: 47, height: 20, fontSize: 10, textAlign: 'center',
    color: '#006DF0', lineHeight: 20, marginLeft: 5, fontFamily: 'Paperlogy-SemiBold'
  },
  photoScroll: { flexDirection: 'row', paddingHorizontal: 24 },
  photoWrapper: { marginTop: 16, width: 130, flexShrink: 0 },
  photo: { width: 130, height: 130, borderRadius: 10 },
  imagePlaceholder: {
    width: 130, height: 130, borderRadius: 10, backgroundColor: '#f0f0f0', justifyContent: 'center', alignItems: 'center',
  },
  placeholderText: { fontSize: 12, color: '#aaa' },
  centered: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#fff' },
  loadingText: { fontSize: 16, fontFamily: 'Paperlogy-Regular', color: '#888' },
  errorText: { fontSize: 16, fontFamily: 'Paperlogy-Bold', color: '#d00', marginBottom: 10 },
  retryText: { fontSize: 14, fontFamily: 'Paperlogy-Regular', color: '#555' },
});
